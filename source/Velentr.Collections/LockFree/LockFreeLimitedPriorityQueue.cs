...
namespace Velentr.Collections.LockFree
{
    ...
    public class LockFreeLimitedPriorityQueue<T, ConverterType> : Collection, IEnumerable<T>, IEnumerable
    {
        ...
        ///     The queues
        /// </summary>
        private readonly Dictionary<ConverterType, LockFreeQueue<T>> _queues;
        ...
        public LockFreeLimitedPriorityQueue(PriorityConverter<ConverterType> converter)
        {
            ...
            this._queues = new Dictionary<ConverterType, LockFreeQueue<T>>(this._converter.OptionCount);
            this._validIntegerValues = new List<int>(this._converter.OptionCount);
            foreach (var option in this._converter.Options)
            {
                this._queues.Add(option.Key, new LockFreeQueue<T>());
