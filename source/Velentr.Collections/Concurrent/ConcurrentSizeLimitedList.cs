...
/// <summary>
/// A thread-safe list with a maximum size limit. When the limit is reached, it performs a specified action to handle the overflow.
/// </summary>
...
public class ConcurrentSizeLimitedList<T> : IList<T>
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ConcurrentSizeLimitedList{T}"/> class with a specified maximum size and action when full.
    /// </summary>
    ...
    {
        _internalList = new SizeLimitedList<T>(maxSize, actionWhenFull, true);
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="ConcurrentSizeLimitedList{T}"/> class with a specified starting capacity,
    ...
    public ConcurrentSizeLimitedList(int startingCapacity, int maxSize, 
        SizeLimitedCollectionFullAction actionWhenFull = SizeLimitedCollectionFullAction.PopOldestItem)
    {
        _internalList = new SizeLimitedList<T>(startingCapacity, maxSize, actionWhenFull, true);
    }
    ...
    public ConcurrentSizeLimitedList(IList<T> baseList, int maxSize, 
        SizeLimitedCollectionFullAction actionWhenFull = SizeLimitedCollectionFullAction.PopOldestItem)
    {
        _internalList = new SizeLimitedList<T>(baseList, maxSize, actionWhenFull, true);
    }

    /// <summary>
    /// Gets the maximum size of the list.
    /// </summary>
    public int MaxSize => _internalList.MaxSize;
    ...
    {
        get => _internalList.ActionWhenFull;
        set => _internalList.ActionWhenFull = value;
    }
    ...
    public T? AddAndReturn(T item) => _internalList.AddAndReturn(item);
    ...
    public List<T> ChangeMaxSize(int newMaxSize) => _internalList.ChangeMaxSize(newMaxSize);

    /// <summary>
    /// Returns an immutable snapshot of the list.
    /// </summary>
    /// <returns>An immutable copy of the current list state</returns>
    public ImmutableList<T> AsImmutable() => _internalList.AsImmutable();
