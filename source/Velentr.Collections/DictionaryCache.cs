...
namespace Velentr.Collections
{
    ...
    public class DictionaryCache<TKey, TValue> : Collection, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable
    {
        ...
        {
            this._dictionary = ImmutableDictionary.Create<TKey, TValue>();
        }
        ...
        /// <summary>
        ///     Performs an implicit conversion from <see cref="DictionaryCache{TKey, TValue}" /> to
        ...
        /// <summary>
        ///     Performs an implicit conversion from <see cref="Dictionary{TKey, TValue}" /> to
        ///     <see cref="DictionaryCache{TKey, TValue}" />.
        /// </summary>
        /// <param name="source">The source.</param>
        /// <returns>
        ///     The result of the conversion.
        ...
        public void Append(Dictionary<TKey, TValue> newItems)
        {
            ...
            do
            {
                oldDictionary = this._dictionary.ToImmutableDictionary();
                newDictionary = oldDictionary.AddRange(newItems);
            } while (!AtomicOperations.CAS(ref this._dictionary, newDictionary, oldDictionary));
        ...
        {
            UpdateCount(-this._dictionary.Count);
        ...
        public bool GetOrUpdate(TKey key, TValue value, out TValue outValue)
        {
            ...
            if (this._dictionary.TryGetValue(key, out var output))
            {
                if (!output.Equals(value))
                {
                    IImmutableDictionary<TKey, TValue> oldDictionary;
        ...
        {
            return this._dictionary.ToDictionary(p => p.Key, p => p.Value);
        }
        ...
        {
            return this._dictionary.TryGetKey(searchKey, out actualKey);
        }
